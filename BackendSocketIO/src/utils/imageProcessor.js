...
exports.getDominantColors = function(base64Image, containerCount, success) {
  ...
  const imgString =
    base64Image.indexOf("base64,") !== -1
      ? base64Image.split("base64,")[1]
      : base64Image;
  // get colors and put them into containers according to hue
  Jimp.read(Buffer.from(imgString, 'base64'), function(err, img) {
    if (err) throw err;
    ...
    for (var x = 0; x < width; x++) {
      for (var y = 0; y < height; y++) {
        ...
        const hue = new Color(
          "rgb(" + clrRgb.r + "," + clrRgb.g + "," + clrRgb.b + ")"
        ).hue();
    ...
    console.log(result);
    console.log(
      "Got dominant colors of an image in",
...
exports.encodeBase64 = function(imageName, maxDimension, callback) {
  Jimp.read("./static/img/" + imageName, function(err, img) {
    if (err) throw err;
    const width = img.bitmap.width;
    const height = img.bitmap.height;
    if (width > height && width > maxDimension) {
      img.resize(maxDimension, Jimp.AUTO);
    } else if (height > MAX_DIMENSION) {
      img.resize(Jimp.AUTO, maxDimension);
    }
